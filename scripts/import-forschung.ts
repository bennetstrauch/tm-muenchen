...
import { loadEnvLocal, readCorpus, isRctMeta, loadDoiMap } from './forschung/source';
...
function toRow(row: (typeof source)[number]) {
  ...
  const doiUrl = overrides[id] ?? inline ?? cache[id] ?? null;
