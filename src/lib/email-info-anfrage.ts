...
const USER_STRINGS: Record<string, InfoAnfrageEmailStrings> = {
  de: {
    ...
    userSignoff: 'Herzliche Grüße,\nDein TM-Team München',
  },
  en: {
  ...
  fr: {
    ...
    userSignoff: 'Chaleureusement,\nTon équipe MT Munich',
  },
  es: {
    ...
    userSignoff: 'Un saludo cálido,\nTu equipo de MT Múnich',
  },
};
...
export function buildInfoAnfrageUserHtml(params: { name: string; locale?: string }): string {
  ...
  const body = `
    <tr>
      <td style="padding:20px;background:#ffffff;font-size:16px;line-height:1.8;color:#555;">
        ...
        <p style="margin:24px 0 0;">${s.userSignoff.replace('\n', '<br>')}</p>
      </td>
    </tr>`;
