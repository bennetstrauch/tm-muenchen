...
async function translateBatch(keysAndValues, locale) {
  ...
  const prompt = [
    `You are a professional copywriter and translator localising the website of TM München, a Transcendental Meditation center in Munich.`,
    `Target language: ${LOCALE_NAMES[locale]}`,
    ``,
    `Tone & style rules:`,
    `- Warm, direct, and conversion-optimised. Write like a trusted friend, not a brochure.`,
    `- Short headline strings (ending in . or ?) must be punchy and idiomatic — do NOT translate word-for-word. Capture the emotional intent.`,
    `- Longer body text: stay close to the original meaning but make sentences feel natural in the target language.`,
    `- Use informal "you" throughout (English: "you", French: "tu", Spanish: "tú").`,
    `- Never add filler words or marketing fluff that isn't in the original.`,
    ``,
    `Technical rules:`,
    `- ICU placeholders like {count} or {current} must appear unchanged.`,
    `- Newlines (\\n) inside strings are layout breaks — preserve them exactly.`,
    `- UI labels (button text, form labels, aria labels) should be concise and action-oriented.`,
    ``,
    `Glossary — always use these exact translations:`,
    glossaryText,
    ``,
    `Translate each value from German. Return ONLY a valid JSON object mapping each key to its translation. No explanation, no markdown, no extra text.`,
    ``,
    `Source strings:`,
